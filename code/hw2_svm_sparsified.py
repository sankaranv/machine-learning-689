...
def main():
    ...
    C_vals = [1e-3, 1e-2, 1e-1, 1, 1e1, 1e2, 1e3]
    test_errors = []
    sparsities = []

    clf_base = SVM(C=1e-1, iterations=10000)

    for C in C_vals:
        clf = SVM(C=C, iterations=10000)
        clf.fit(train_X, train_y)
        print(clf.w)
        clf.w[np.abs(clf.w) < 1e-4] = 0
        print(clf.w)

        pred_y_test = clf.predict(test_X)
        print("Avg test prediction error: " +
              str(zero_one_loss(test_y, pred_y_test)))
        test_errors.append(zero_one_loss(test_y, pred_y_test))

        sparsity = np.count_nonzero([clf.w == 0]) / clf.w.shape[0]
