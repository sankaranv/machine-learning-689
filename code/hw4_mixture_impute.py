...
class mixture_model(nn.Module):
    ...
    def fit(self, X, mu_init=None, b_init=None, pi_init=None, step=0.1, epochs=100):
        ...
        for epoch in range(epochs):
            ...
            if(np.abs(loss - loss_prev) < 0.001):
                break
            loss_prev = loss
...
def main():
    ...
    xtr2_zero = np.copy(xtr2)
    xtr2_zero[np.isnan(xtr2_zero)] = 0
    xte2_zero = np.copy(xte2)
    xte2_zero[np.isnan(xte2_zero)] = 0
    Xref = xtr2_zero + xte2_zero
    ...
    for K in range(1, 21):
        ...
        mean_abs_impute_error = 1
        for i in range(10):
            mm = mixture_model(K=K)
            mm.fit(xtr2)
            train_likelihood = mm.marginal_likelihood(xtr2)
            print("Training Data: run " + str(i + 1) +
                  " : Likelihood = " + str(train_likelihood))
            if(train_likelihood > best_likelihood):
                best_likelihood = train_likelihood
                test_likelihood = mm.marginal_likelihood(xte1)
                XI = mm.impute(xtr2)
                mean_abs_impute_error = np.mean(np.abs(XI - Xref))
        print("Best Likelihood on incomplete training data: " + str(best_likelihood))
        print("Likelihood on test data: " + str(test_likelihood))
        print("Mean absolute imputation error: " + str(mean_abs_impute_error))
        train_likelihoods.append(best_likelihood)
        test_likelihoods.append(test_likelihood)
        impute_errors.append(mean_abs_impute_error)
    ...
    plt.title("Log Marginal Likelihood vs K on training and test data")
    plt.xlabel('Number of mixture components (K)')
    plt.ylabel('Log Marginal Likelihood')
    plt.xticks(axis)
    plt.savefig('hw4_q3_combo.png')
    plt.clf()
    plt.plot(axis, impute_errors, color='green')
    plt.title("Mean absolute imputation error vs K")
    plt.xlabel('Number of mixture components (K)')
    plt.ylabel('Mean absolute imputation error')
    plt.savefig('hw4_q3_impute.png')
